 ...
 */
export const gapFillCharges = onCall({ cors: true, region: "europe-west1" }, async (request) => {
  ...
  for (const doc of chargesSnapshot.docs) {
    ...
    if (visitId) {
      ...
      if (potentialMatch) matchingTripId = potentialMatch.id;
    }

    // B: Match by Heuristic
    if (!matchingTripId && shipName && chargeDate) {
      const start = new Date(chargeDate); start.setHours(start.getHours() - 24);
      const end = new Date(chargeDate); end.setHours(end.getHours() + 24);
    ...
    if (matchingTripId) {
       const updatePayload: any = {
        isConfirmed: true,
        confirmedAt: charge.created_at || charge.date || FieldValue.serverTimestamp(),
      ...
      const newTrip = {
        ...
        shipName: shipName,
        gt: charge.gt || 0,
        boarding: chargeDate ? Timestamp.fromDate(chargeDate) : Timestamp.now(),
        isConfirmed: true,
        confirmedAt: charge.created_at || charge.date || Timestamp.now(),
        confirmedBy: charge.user_name || charge.createdBy || 'Legacy System (GapFill)',
...
function mapChargeTypeToTripType(chargeType: string): string {
  const norm = chargeType.toLowerCase();
  if (norm.includes('in')) return 'In';
  if (norm.includes('out')) return 'Out';
  if (norm.includes('shift')) return 'Shift';
  if (norm.includes('anchor')) return 'Anchorage';
  return 'Other';
}
