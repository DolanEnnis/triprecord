 ...
 */
export const onShipUpdated = onDocumentUpdated("ships/{shipId}", async (event) => {
  const shipId = event.params.shipId;
  const oldShip = event.data?.before.data();
  const newShip = event.data?.after.data();

  // 1. Safety Checks
  if (!oldShip || !newShip) return;
  ...
  const visitsToUpdate: admin.firestore.DocumentReference[] = [];
